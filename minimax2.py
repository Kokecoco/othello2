...
class Minimax2AI(AI):
    ...
    def get_move(self, moves, board, stone):
        ...
        # テンソルの処理
        board_array = self._process_board(board)

        best_move = None
        best_value = float("-inf")

        for move in moves:
            new_board = self.make_move(board_array, move, stone)
    ...
    def _process_board(self, board):
        """ボード状態の前処理"""
        try:
            if tf.is_tensor(board):
                board = board.numpy()
            if len(board.shape) == 3:  # (8, 8, 1)の形状を処理
    ...
    def make_move(self, board, move, stone):
        """ボードの状態を更新（テンソル対応）"""
        try:
            board = self._process_board(board)
